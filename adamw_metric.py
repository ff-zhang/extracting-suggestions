...
def create_optimizer(init_lr, num_train_steps, num_warmup_steps, end_lr=0.0, beta_1=0.9, poly_power=1.0):
    """Creates an optimizer with learning rate schedule."""
    # Implements linear decay of the learning rate.
    lr_schedule = tf.keras.optimizers.schedules.PolynomialDecay(initial_learning_rate=init_lr,
...
class WarmUp(tf.keras.optimizers.schedules.LearningRateSchedule):
    ...
    def __call__(self, step):
        with tf.name_scope(self.name or 'WarmUp') as name:
            ...
            return tf.cond(global_step_float < warmup_steps_float, lambda: warmup_learning_rate,
                           lambda: self.decay_schedule_fn(step), name=name)
