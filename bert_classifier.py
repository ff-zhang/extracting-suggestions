...
def train_bert(ds_list: list[tf.data.Dataset], logs_dir: pathlib.Path, **params):
    ...
    # save_graph(logs_dir / 'graphs' / f'{f1}.png', history)
    save_graph(logs_dir / 'graphs' / 'bert.png', history)
...
if __name__ == '__main__':
    ...
    ds_list = [ds.with_options(options).batch(params['BATCH_SIZE']).cache().prefetch(tf.data.AUTOTUNE) for ds in ds_list]
